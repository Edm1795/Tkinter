...
class MainWindow:
    def __init__(self, master):
        ...
        # Master Window
        self.master = master
        self.master.title('Supply Chain Ver. 0.9')
        ...
        self.speedometerDict = {'Blue Pens':[[1,1],[1,2],501],'Staedtler Pens':[[1,3],[1,4],411],'China Markers':[[1,5],[1,6],401],'Red Pens':[[1,7],[1,8],579],'Paper Clips':[[1,9],[1,10],503],'White Tape':[[1,11],[1,12],501],'Golf Pencils':[[1,13],[1,14],401],'Scissors':[[2,1],[2,2],421],'Transparent Name Tag Slips (Small)':[[2,3],[2,4],579]}
    ...
    def editItem(self):
        ...
        editItemLabel = Label(top, text="New Quantity of Stock", font=('Cambria 12'))  # Label(top, text="Add New Item", font=('Mistral 18 bold')).place(x=150, y=80)
        editItemLabel.grid(row=1,column=1)  # Note: .grid cannot be placed as a single line code: Label(...).grid(..) as the .grid will actually return None to the program and casue an error
        editItemEntry = Entry(top, width='40')
        editItemEntry.grid(row=1, column=2)
        editItemEntry.bind('<Return>', lambda event: updateQuantityInSpeedometerDict())

        def updateQuantityInSpeedometerDict():
    ...
    def drawCircle(self,itemName, LabRowCol,cirRowCol):
        ...
        editButton = Button(c, text='Edit', command=self.editItem)                                                     # create button on the canvas
    ...
    def updateSpeedometer(self, speed, XcircleOrigin, YCircleOrigin, radius,listIndexCounter):
        ...
        for i in range(0, len(list1),2):  # move in steps of two, 0,2,4,6
            ...
            self.canvases[listIndexCounter].create_line(XcircleOrigin, YCircleOrigin,nextTup[0],nextTup[1], fill='#'+str(colour), width=3)  # draw line in circle; values: x and Y of both  ends of line (1st point: horiz, verti, end point horiz, vert.
            self.master.update()
