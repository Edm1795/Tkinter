...
class MainWindow:
    ...
    def addItem(self):
        ...
        def addMaxStockValue(): # add max stock to self.stockAndUnitValueDict
            ...
            assert (str.isdigit(setMaxStockEntry.get())), "Please input digits only."
            ...
            self.updateConfigFile()  # Update config file (moved from above so that dict is updated first, then conifg file is updated

            completionMes = Label(top, text=setMaxStockEntry.get() + ' set as max stock', font=('Cambria 14'))
            completionMes.grid(row=4, column=1)
    ...
    def updateStockAndUnitValueDict(self,maxStock,nameOfItem): # put max stock value into StockAndUnitValueDict
        ...
        print('stockAndUnitValueDict contents',self.stockAndUnitValueDict.items())
    ...
    def updateConfigFile(self):  # See loadSpeedomterDict for other side of process
        ...
        with open('config.txt', 'wb') as f: # Open in wb mode which deletes all contents previously saved and will write new contents
            ...
            print('updating config file',self.speedometerDict.items())
            f.close()
        with open('config2.txt', 'wb') as f2:
            p.dump(self.stockAndUnitValueDict, f2)
            print('updating config2 file',self.stockAndUnitValueDict.items())
            f2.close()
