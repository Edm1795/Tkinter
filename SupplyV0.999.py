...
class MainWindow:
    ...
    def __init__(self, master):
        ...
        # Master Window
        self.master = master
        self.master.title('Supply V. 0.999')
...
def editItem(itemName):
    ...
    editItemLabelFuStock = Label(top, text='(Full Stock = ' + str(stockAndUnitValueDict[itemName][0])  + ' ' + stockType  + ')', font=('Cambria 12')) # Label displaying value for full stock
    ...
    def updateQuantityInSpeedometerDict(nameOfStock,item,quantity):
        ...
        try:
            ...
            # gridPos=getLabelGridPos(nameOfStock)
            # mainWin.drawCircle(nameOfStock,[gridPos[0],gridPos[1]],[gridPos[0],gridPos[1]+1])

            # canGridPos = getCanvasGridPos(itemName)
            # print(canGridPos)
            # canIndValue = canGridPos[1] - 1
            # print(canIndValue)
            circVals=mainWin.getCircleValues()
            # print(circVals)
            mainWin.calcSpeed(itemName)
...
def editFullStock(itemName):
    ...
    def editFullStockProp(itemName,maxStock):
        ...
        stockAndUnitValueDict[itemName][0] = int(maxStock)
...
def showHistory():
    ...
    for object in historyList:
        if object.getAction()=='Add':
            ...
            histLabList.append(Label(top, text=object.getName() + ' edited to ' +object.getNewVal(), font=('Cambria 12')))  # Label(top, text="Add New Item", font=('Mistral 18 bold')).place(x=150, y=80)
            ...
            histLabList.append(Label(top, text=object.getName() + ' renamed to ' + object.getNewVal(), font=('Cambria 12')))  # Label(top, text="Add New Item", font=('Mistral 18 bold')).place(x=150, y=80)
            histLabList.append(Label(top, text=object.getTime(), font=('Cambria 12'), fg='blue'))
    ...
    # Accessing list thus: 0,1 ; 2,3 ; 4,5
    while c != len(histLabList):
        histLabList[c].grid(row=rowNum,column=0) # Access Label for action
        histLabList[c+1].grid(row=rowNum,column=2) # Acess Label for time
        rowNum+=1 # Go to next row
        c+=2 # up by 2 to get to next pair
