...
class MainWindow:
    def __init__(self, master):
        ...
        self.drawCircle(10, 10, 80, 80)  # draws circle according to coordinates of containing box. Be sure to call self.circleOrigin with the SAME values so that the spedmeter line fits correctly in the circle
        self.circleOrigin = self.calcCircleOrigin(10, 10, 80, 80)  # self.circleOrigin: a list of spedmeter's values: [xOrigin, yOrigin, radius]
    ...
    def drawCircle(self,x1,y1,x2,y2):
        ...
        self.label = Label(self.frame2, text='Black Pens')
        # self.label.pack()
    ...
    def updateSpedometer(self, speed, radius, XcircleOrigin, YCircleOrigin):
        ...
        for i in range(0, len(list1),2):  # move in steps of two, 0,2,4,6
            ...
            self.c.create_line(XcircleOrigin, YCircleOrigin,nextTup[0],nextTup[1], fill='purple', width=2)  # draw line in circle; values: x and Y of both  ends of line (1st point: horiz, verti, end point horiz, vert.
            self.master.update()
